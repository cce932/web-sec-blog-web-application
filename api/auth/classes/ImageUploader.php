...
class ImageUploader
{
  ...
  private function checkInitialBytes($image)
  {
    ...
    $contents = file_get_contents($image['tmp_name'], false, null, 0, 100);
  ...
  private function saveImageLinkIntoDb($image, $username) {
    ...
    if (!$this->db_connection->connect_errno) {
      $image_link = $this->db_connection->real_escape_string($image);

      $sql = "UPDATE users
              SET img_link='" . $image_link . "' WHERE username='" . $username . "';";
      $query_new_link_insert = $this->db_connection->query($sql);

      if (!$query_new_link_insert) {
        throw new Exception("Sorry, your registration failed. Please go back and try again.");
  ...
  public function uploadImageLink($image, $username)
  {
    $this->saveImageLinkIntoDb($image, $username);
   ...
   */
  public function uploadImageFile($image, $identifier, $callback = null) // $identifier now is $username passed by register.php
  {
  ...
  private function serveImageLink($user_id) {
    ...
    if (!$this->db_connection->connect_errno) {
      ...
      if ($result_of_img_link_check->num_rows == 1) {
        $result_row = $result_of_img_link_check->fetch_object();
        return $result_row->img_link;
      } else {
        return false;
      }
    }
